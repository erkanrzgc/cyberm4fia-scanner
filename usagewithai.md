...
| `--dorking` | Automated Google Dorking for target |
| `--wayback` | Wayback Machine historical URL discovery |
| `--urlscan` | URLScan.io passive reconnaissance |
| `--rotate-proxy` | Rotate proxies from a live pool for each request (auto-enables with WAF evasion) |
| `--har-output` | Record browser traffic as HAR file and analyze for API endpoints |
